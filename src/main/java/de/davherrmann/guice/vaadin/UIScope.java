	...
	public <T> Provider<T> scope(final Key<T> key, final Provider<T> unscoped) {
		return new Provider<T>() {
			public T get() {
				...
				log.debug("looking for a UIScoped instance of {}", key
						.getClass().getName());

				// get the current UIKey. It should always be there, as it is
				// created before the UI
				UIKey uiKey = CurrentInstance.get(UIKey.class);
				// this may be null if we are in the process of constructing the
				// UI
				...
				if (uiKey == null) {
					if (currentUI == null) {
						throw new UIScopeException("UI and uiKey are null. "
								+ msg);
					} else {
						...
						if (uiKey == null) {
							throw new UIScopeException(
									"uiKey is null and cannot be obtained from the UI. "
											+ msg);
						}
					}
				}

				// currentUI may be null if we are in the process of
				// constructing the UI
				// if not null just check that it hasn't got out of sync with
				// its uikey
				if (currentUI != null) {
					if (!uiKey.equals(currentUI.getInstanceKey())) {
						throw new UIScopeException(
								"The UI and its UIKey have got out of sync.  Results are unpredictable. "
										+ msg);
					}
				}
				...
				// this line should fail tests but having trouble setting up a
				// decent test. TestBench needed?
				// Map<Key<?>, Object> scopedObjects =
				// getScopedObjectMap(CurrentInstance.get(UIKey.class));
				...
				if (current != null) {
					log.debug("returning existing instance of "
				...
				log.debug("new instance of "
						+ current.getClass().getSimpleName()
						+ " created, as none in cache");
				return current;
			}
		};
	}
